...
// TODO consider using positionconversionfactor to make units of arm rotation as opposed to motor rotations
...
public class Arm extends SubsystemBase {
    ...
    public Command pidControl(ArmState state) {
        return runOnce(() -> m_PID.setSetpoint(state.kPosition)).andThen(run(
                () -> {
    ...
    public Command autoHome() {
        // TODO make a command using current detection to stop the arm at the bottom and redefine the position as 0 (or whatever angle it should be).
        // You could (should) also be checking the current spike in periodic and stopping the motors if something is awry.
